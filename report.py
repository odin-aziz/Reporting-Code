...
def clean_and_convert_gmv(df, sum_column='GMV'):
    ...
    if sum_column in df.columns:
        ...
        st.error(f"Error: '{sum_column}' column is missing in the dataframe.")
    return df
...
if uploaded_file_week1 and uploaded_file_week2:
    ...
    # Debugging: Show the columns of the uploaded files
    st.write("### Columns in Week 1 File")
    st.write(df_week1.columns)
    
    st.write("### Columns in Week 2 File")
    st.write(df_week2.columns)
